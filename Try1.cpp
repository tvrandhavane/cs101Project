...
#include "City.h"
#include "Company.h" 
SimpleWindow w("M1",18.0,12.0,Position(0.0,0.0));
...
{	
	...
	while(!feof(fp2))
	{
		fscanf(fp2,"%s %s %d %d %d %d %d %d",str1,str2,&id,&p_comp_id,&comp_cost,&b_rent,&p_rent,&c_owner);
		cmpny[i].set_details_of_company(str1,str2,id,p_comp_id, b_rent, p_rent,comp_cost,c_owner);
		i++;
	}
	for(i=0;i<2;i++)
	{
		plyr[i].set_details_of_player(i);
	}
	fclose(fp1);
	fclose(fp2);
	...
	char ch;												//flag=0 denotes city, and flag=1 denotes company
	                                                                      //main algorithm
	...
	while((plyr[0].return_balance()>0)&&(plyr[1].return_balance()>0))
	{
		flag=-1;
		i=z%2;
		z++;
		...
		{
			cout<<"\nYou have landed at 'COMMUNITY CHEST'.";
			switch (dice_sum)
			{
				case 2:
